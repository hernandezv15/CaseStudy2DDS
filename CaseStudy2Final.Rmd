...
roc_curve <- roc(testData_processed$Attrition, predicted_probabilities[, "Yes"])
...
final_predictions <- ifelse(predicted_probabilities[, "Yes"] > 0.53, "Yes", "No")
...
output
write.csv(output, "attrition_predictions.csv", row.names = FALSE)
```
...
model_MI <- lm(MonthlyIncome ~ ., data = train_data_income)
summary(model_MI)
...
outputMI
write.csv(output, file = "predicted_salaries.csv", row.names = FALSE)
